...
function! s:RefreshCurrent(rBrowser)
  if s:IsOpen()
    ...
    "MH: All config stuff in a global variable
    let g:ProjConfig = s:Current

    "Added by MH
    if has_key(s:Current, 'exec') == 1
      exec s:Current['exec']
    end
    ...
    "MH: opens sesson with curent project
    "I do not want to use the sessino automaticaly:
    "I try to use workspaces (ctrl-space).
    "call g:SessionExecute("OpenSession")
    ...
    " Load last workspace
    exe "CtrlSpaceLoadWorkspace"

    "save_workspace_on_exit = 1 didn't work for me :(
    "let g:ctrlspace_save_workspace_on_exit = 1
    " unmap <Space><F4>
    " map <Space><F4> :CtrlSpaceSaveWorkspace<CR>:qa<CR>
    

    "exe "silent! :CtrlSpace"
    "exe "silent! normal w<Space>"
    "exe ":CtrlSpace"
    "It always created one empty tab (did appedn to current bufferes)
    "if bufnr('$') > 1
        "exe "normal 1gt"
        "exe ":q"
    "end
...
call s:Set('g:ProjFileBrowser', 'off')
...
function! s:PromptOpen()
  let name = input('Open project: ', '', 'customlist,g:ProjComplete')
  if len(name)
...
function! s:PromptOpenTab()
  let name = input('Open project in new tab: ', '', 'customlist,g:ProjComplete')
  if len(name)
