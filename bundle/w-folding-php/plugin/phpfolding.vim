...
function! s:PHPCustomFolds() " {{{
	...
	call s:PHPFoldPureBlock('function [a-zA-Z0-9]\+ \?(', s:FOLD_WITH_PHPDOC)
    ...
    " //MH
        ...
        " call s:PHPFoldProperties('^\s*const\s[A-Z0-9]\+', ";", s:FOLD_WITH_PHPDOC, 1, 1)
...
augroup SetPhpFolds
    au!
    au BufWinEnter * call s:CheckAutocmdEnablePHPFold()
augroup END
" }}}
